...
const login = async (req, res) => {
    ...
    await general
        .raw("Select * From users Where username = ? or email = ?", [
            username,
            password,
        ])
        .then(async (response) => {
            ...
            try {
                ...
                if (env.debug) {
                    console.log(
                        `:rocket: ~ file: auth.js ~ line 42 ~ awaitgeneral.raw ~ password`,
                        password
                    );
                    console.log(
                        `:rocket: ~ file: auth.js ~ line 43 ~ awaitgeneral.raw ~ resp.password`,
                        resp.password
                    );
                }
                ...
                console.log(
                    `:rocket: ~ file: auth.js ~ line 41 ~ awaitgeneral.raw ~ error`,
                    error
                );
            ...
            if (!resp) {
                return res
                    .status(400)
                    .json(
                        message(true, resp, "no se pudo encontrar registros")
                    );
            } else {

                const menu = general
                    .raw("Select menu, link, icon From view_menu Where username = ?", [
                        resp.username
                    ]).then(data => parseDataKnex(data))

                const privileges = general
                    .raw("Select permission, name From view_privileges Where username = ?", [
                        resp.username
                    ]).then(data => JSON.parse(parseDataKnex(data)[0].permission))

                const information = general
                    .raw("Select * From view_information_users Where username = ?", [
                        resp.username
                    ]).then(data => parseDataKnex(data))
        ...
        .catch((err) => {
            console.log(
                `:rocket: ~ file: auth.js ~ line 66 ~ login ~ err`,
                err
            );
            return res
                .status(500)
