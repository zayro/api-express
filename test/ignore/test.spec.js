...
connection_defalt('users').select('username', 'password').then(response => {
        //console.log(`:rocket: ~ file: auth.js ~ line 15 ~ awaitgeneral.raw ~ resp`, JSON.parse(JSON.stringify(response)));
...
connection_defalt.raw("select * from  users limit 1 ").then(response => {
        console.log(`:rocket: ~ file: auth.js ~ line 15 ~ awaitgeneral.raw ~ resp`, JSON.parse(JSON.stringify(response[0])));
